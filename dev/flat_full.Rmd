...
test_that("read_biotyper_report is empty when no peaks are found", {
  ...
  expect_warning(
    read_biotyper_report(biotyper_empty),"due to no peaks found"
    )
})
```
...
test_that("check_spectra works", {
  expect_type(
    check_spectra(
      c(createMassSpectrum(1:3, 1:3), createMassSpectrum(11:13, 11:13))
    ), "list"
  )
})
test_that("check_spectra warns with empty spectra", {
  expect_warning(
    check_spectra(createMassSpectrum(mass = 0, intensity = 0)),
    "are incorrect (empty, outlier length or irregular)"
  )
})
```
