...
#' * A list of **character** that are the paths to RDS files produced by [process_spectra] when using `rds_prefix`.
...
merge_processed_spectra <- function(processed_spectra, remove_peakless_spectra = TRUE, interpolate_missing = TRUE) {
  if(any(is.null(processed_spectra), !is.list(processed_spectra), length(processed_spectra) == 0)){
    stop(
  ...
  } else if (typeof(processed_spectra[[1]]) == "list"){
