...
The function, as illustrated in the example below, can pick spectra using different types of inputs:

* the reference spectra information that is present in the cluster table (after using `similarity_to_clusters` or  `import_spede_clusters` functions)
* an external metadata table containing a variable (e.g., optical density, fluorescence) to be maximized (default) or minimized per cluster
* a regular expression applied to the spectra names to explicitly removed some spectra and therefore clusters from the picking decision. This is particularly useful if some spectra  have been previously picked. For example, the regular expression could match a date or time to exclude colonies grown and picked 24h after streaking when comparing with colonies grown for 72h, or match a growth condition such as a temperature or an antibiotic provided this information is in the spectra name. A dry-run option is provided to display the spectra to be removed with the regular expression _without actually performing  the picking decision_.
...
pick_spectra <- function(
  ...
  cluster_df %>%
    dplyr::ungroup() %>%
    dplyr::right_join(orig_cluster_df) %>%
    dplyr::mutate(
...
# 0. Load a toy example of a tibble of clusters created by the `similarity_to_clusters`
#  function.
clusters <- readRDS(system.file("clusters_tibble.RDS", package = "maldipickr"))

# 1. By default and if no other metadata are provided, the function
...
# 2.2 Pick the spectra based on the highest OD600 value per cluster
...
metadata <- metadata %>% mutate(
  well = gsub(".*[A-Z]([0-9]{1,2}$)", "\\1", name) %>% strtoi(),
...
# 3.2 
pick_spectra(clusters, metadata, "OD600", "is_edge")

# Say that all spectra in the column F were picked before
#   in a previous experiment. We do not want to pick clusters
#   with those spectra included to limit redundancy.
metadata <- metadata %>% mutate(
  picked_before = grepl("_F", name)
)
pick_spectra(clusters, metadata, "OD600", hard_mask_column = "picked_before")
```
