...
#' Cherry-pick Bruker MALDI Biotyper spectra
#' 
#' If no metadata/criteria is provided, the reference spectra would be picked
#' 
...
#' 
#' @export
pick_spectra <- function(cluster_df, metadata_df = NULL, discard_regex = NULL, is_sorted=FALSE) {
# Check that the mandatory column describing the cluster are present
  ...
  original_cluster_df <- cluster_df
  cluster_df <- rowid_to_column(cluster_df, "rowid")
  ...
  if (!is.null(discard_regex)) {
    clusters_to_keep <- cluster_df %>%
      ...
      dplyr::mutate(
        "to_discard" = base::grepl(.data$name, discard_regex)
        ) %>%
      dplyr::group_by(.data$membership) %>%
      ...
      dplyr::filter(n == 1 & !to_discard) %>%
      dplyr::pull(.data$membership)
    if(only_show_discarded){
      message("The following clusters are discarded:")
  ...
  cluster_df %>%
    ...
    tibble::rowid_to_column() %>% 
      dplyr::group_by(.data$membership) %>%
    # dplyr::arrange(desc(.data$OD600), .by_group = TRUE) %>%
  }
  ...
  # end merge the name, to_pick T/F with the original cluster_df
}
```
...
set.seed(104)
metadata <- dplyr::transmute(clusters, name = name, OD600 = runif(n = n()))

species2_E12 was picked before
pick_spectra(clusters, metadata)
```
  
```{r tests-pick_spectra}
test_that("pick_spectra works", {
  expect_true(inherits(pick_spectra, "function")) 
})
```
