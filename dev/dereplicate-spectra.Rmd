...
remove_spectra <- function(spectra_list, to_remove) {
  if (is_a_processed_spectra_list(spectra_list)) {
    ...
    # spectra_list[["spectra"]], to_remove
    # spectra_list[["peaks"]], to_remove
    # spectra_list[["metadata"]][!to_remove, ]
    # could fall back to the function below for raw spectra but used on spectra, peaks after converting the names in logical
  } else {
    # spectra_list is a raw spectra
    # to_remove should be logical vector of same length as
    if (base::unlist(to_remove) %>% base::class != "logical") {
      stop(
        "remove_spectra() with raw spectra expects logical in 'to_remove'",
        "You could provide the output of `check_spectra()`."
      )
    }
    to_remove <- switch(base::class(to_remove),
      "list" = Reduce(any, to_remove),
      "logical" = to_remove
    )
...
test_that("remove_spectra works", {
  expect_true(inherits(remove_spectra, "function"))
})
```
