...
identification_to_clusters <- function(tibble_report) {
  ...
  if (n_target_names != nrow(tibble_report)) {
    warning(
      ...
      "Did you forget to use best_hits = TRUE or to filter the hits?"
    )
  }
  ...
  tibble_report %>%
    dplyr::mutate(
      ...
      "membership" = as.numeric(as.factor(.data$membership))
    ) %>%
    dplyr::group_by(.data$membership) %>%
    dplyr::mutate(
      "cluster_size" = dplyr::n()
...
test_that("identification_to_clusters works", {
  expect_true(inherits(identification_to_clusters, "function"))
})
```
